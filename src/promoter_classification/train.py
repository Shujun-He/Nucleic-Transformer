...
def train_fold():
    ...
    df=pd.read_csv('v9d3.csv')

    sequences=np.asarray(df.seqs)
...
#                scaled_loss.backward()
        ...
        val_loss,val_acc,val_sens,val_spec=validate(model,device,val_dataloader,batch_size=opts.batch_size*2)
        print("Epoch {} train acc: {}".format(epoch+1,train_acc))

        to_log=[epoch+1,train_loss,train_acc,val_loss,val_acc,val_sens,val_spec]
        logger.log(to_log)
