...
def train_fold():
    ...
    model=DeePromoter([27, 14, 7]).to(device)
    #exit()
    optimizer=torch.optim.Adam(model.parameters(), lr=1e-3, eps=1e-07)
...
#     model, optimizer = amp.initialize(model, optimizer, opt_level=opt_level)
    ...
    for epoch in range(opts.epochs):
        ...
        for step,data in enumerate(dataloader):
            ...
            #data=dataset[step]
            src=data['data'].float()
            #src=torch.nn.functional.one_hot(src,num_classes=4).float()
...
#                scaled_loss.backward()
            ...
            print ("Epoch [{}/{}], Step [{}/{}] Loss: {:.3f} Lr:{:.6f} Time: {:.1f}"
                           .format(epoch+1, opts.epochs, step+1, total_steps, total_loss/(step+1) , 1e-3,time.time()-t),end='\r',flush=True) #total_loss/(step+1)
            #break
        print('')
