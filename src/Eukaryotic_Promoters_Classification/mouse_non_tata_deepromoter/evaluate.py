...
def evaluate_fold(fold):
    ...
    train_indices, val_indices, test_indices=iter_split_strict(sequences,labels,fold,opts.nfolds)
    # print(train_indices.shape)
    # print(val_indices.shape)
    # exit()
    dataset=PromoterDataset(sequences[train_indices],labels[train_indices])
    val_dataset=PromoterDataset(sequences[test_indices],labels[test_indices])
    ...
    #init model
    model=DeePromoter([27, 14, 7]).to(device).to(device)
    ...
    predictions,attention_weights,sequences,ground_truths=predict(model,device,val_dataloader)
