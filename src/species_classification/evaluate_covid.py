...
labels=np.ones(len(seqs)).astype("int")


batch_size=256
batches=np.around(len(seqs)/batch_size+0.5).astype('int')
preds=[]
...
acc=metrics.accuracy_score(labels,np.argmax(preds,axis=1))

with open("covid_results.p",'wb+') as f:
    pickle.dump([labels,preds],f)


print(acc)
with open("covid_score.txt",'w+') as f:
    f.write("test acc score: {}".format(acc))
