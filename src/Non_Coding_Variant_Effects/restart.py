...
def get_args():
    ...
    parser.add_argument('--restart_epoch', type=int, default=150, help='epoch to restart from')
...
#model.load_state_dict(torch.load(f'checkpoints_fold{opts.fold}/epoch{opts.restart_epoch}.ckpt'))
model.load_state_dict(torch.load(f'epoch{29}.ckpt'))
lr_schedule.step_num=49*len(dataloader)
pytorch_total_params = sum(p.numel() for p in model.parameters())
print('Total number of paramters: {}'.format(pytorch_total_params))

#print("Starting training for fold {}/{}".format(opts.fold,opts.nfolds))
#training loop
#epoch=opts.restart_epoch
for epoch in range(49,opts.epochs):
    model.train(True)
    t=time.time()
    total_loss=0
    optimizer.zero_grad()
    ...
    #for step in range(1):
        ...
        loss=criterion(output.reshape(-1),labels.reshape(-1))
