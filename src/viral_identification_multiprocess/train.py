...
def train_fold():
    ...
    dataset=ViraminerDataset(df_path)
    val_df_path=os.path.join(opts.path,'fullset_validation.csv')
    val_dataset=ViraminerDataset(val_df_path)
    ...
    for epoch in range(opts.epochs):
        ...
        #for step in range(1):
            ...
            output,attention_weights=model(src,None)
            loss_weight=torch.ones(len(output),device=device)
            loss_weight[labels==1]=opts.viral_loss_weight
            ...
            for lab in labels:
                ground_truths.append(lab.cpu().numpy())
        ...
        if (epoch+1)%opts.val_freq==0:
            val_loss,auc,val_acc,val_sens,val_spec=validate(model,device,val_dataloader,batch_size=opts.batch_size)
